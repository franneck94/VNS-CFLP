...
test_set = [test for test in os.listdir(path) if test not in 
    ["capa", "capb", "capc", "cap16", "cap17", "cap18", "cap19"] and "cap" in test]
#test_set = ["cap16"]
...
for testset in test_set:
    ...
    runtime = m.runtime
    m.write(dest+"/"+testset+"/_solution"+testset+".sol")
    # Append Runtime to File
    with open(dest+"/"+testset+"/_solution"+testset+".sol", "a") as f:
